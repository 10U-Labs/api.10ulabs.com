...
A separate job reports as a separate check, so a red run says which rule was broken without reading a log, and the asserts run in parallel with the lint instead of gating it.

Why each assert runs as an action is [an-action-is-preferred-to-a-package-install](an-action-is-preferred-to-a-package-install.md); what the asserts protect is the CI-only verification in [ci-is-the-source-of-truth](ci-is-the-source-of-truth.md).
