...
locals {
  function_name = module.common.lambda_handler_names.hyperscale_cloud_service_provider_regions
...
resource "aws_lambda_function" "handler" {
  ...
  timeout          = 10
  memory_size      = 128
  description      = "Hyperscale cloud service provider regions endpoint: list the regions in the store, create one, and serve, correct or remove one by its id."
