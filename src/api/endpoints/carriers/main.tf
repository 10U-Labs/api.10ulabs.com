...
resource "aws_lambda_function" "handler" {
  ...
  timeout          = 10
  memory_size      = 128
  description      = "Carriers endpoint: list the carriers in the store, create one, serve, rename or delete one by its id, list or add its PoPs or serve, correct or remove one by its id, and list its fiber segments."
