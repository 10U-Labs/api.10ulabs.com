...
from lambda_http import aws_client, json_response, parse_object
from rack_configurations import COLLECTION, configuration_hash, error, logger, served
...
def _store(config_hash: str, configuration: Dict[str, Any], device_id: str) -> None:
    ...
    try:
        ...
        if failure.response['Error']['Code'] != 'ConditionalCheckFailedException':
            raise
...
def _create(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not device_id:
        return error(400, 'Missing required field: device_id')
    if not isinstance(configuration, dict) or not configuration:
        return error(400, 'Missing required field: configuration')
    ...
    try:
        ...
        logger.error('Error storing rack configuration: %s', failure)
        return error(500, 'Failed to store the configuration')
    return json_response(200, {'success': True, 'config_hash': config_hash})


def lambda_handler(event: Dict[str, Any], _context: Any) -> Dict[str, Any]:
    return served(event, {(COLLECTION, 'POST'): _create})
