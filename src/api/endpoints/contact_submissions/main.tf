...
locals {
  ...
  contact_email  = "contact@${module.common.domain_name}"
  parameter_name = "/${module.common.product}/contact/recaptcha-secret-key"
}
...
resource "aws_lambda_function" "handler" {
  ...
  timeout          = 10
  memory_size      = 128
  description      = "Contact submissions endpoint: verify the reCAPTCHA token and send the submission over SES."

  environment {
    variables = {
      AWS_USE_FIPS_ENDPOINT           = "true"
      CONTACT_EMAIL                   = aws_ses_email_identity.contact.email
