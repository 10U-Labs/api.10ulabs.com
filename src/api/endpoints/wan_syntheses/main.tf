...
resource "aws_lambda_function" "handler" {
  ...
  timeout          = 10
  memory_size      = 128
  description      = "WAN syntheses endpoint: list the syntheses in the store, serve one by its id, list a WAN's PoPs or serve one by its id, list a WAN's backbone and homing circuits and the fiber it rides, list a run's sites or serve one by its id, and list a run's regions or serve one by its id."
