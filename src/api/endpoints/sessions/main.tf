...
locals {
  ...
  export_name    = "${local.function_name}-export"
  scheduler_name = "${local.function_name}-scheduler"
  table_name     = "${module.common.product}-session-events"
  bucket_name    = "${module.common.product}-sessions-analytics"
  backup_name    = "${module.common.product}-sessions-backup"
}
...
resource "aws_lambda_function" "handler" {
  ...
  timeout          = 10
  memory_size      = 128
  description      = "Sessions endpoint: record a session's analytics events."
