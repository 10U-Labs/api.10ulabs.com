...
locals {
  ...
  openapi_spec = templatefile("${path.module}/../../../www/openapi.json", {
    ...
    ListBackboneCircuitsHandlerArn    = local.integration.list_backbone_circuits
    ListCarriersHandlerArn            = local.integration.list_carriers
    ListDegreeExemptWanPopsHandlerArn = local.integration.list_degree_exempt_wan_pops
    ListFiberSegmentsHandlerArn       = local.integration.list_fiber_segments
    ListForcedCircuitsHandlerArn      = local.integration.list_forced_circuits
    ListForcedHomesHandlerArn         = local.integration.list_forced_homes
    ListForcedWanPopsHandlerArn       = local.integration.list_forced_wan_pops
    ListHomingCircuitsHandlerArn      = local.integration.list_homing_circuits
    ListOffNetHandlerArn              = local.integration.list_off_net
    ListPopsHandlerArn                = local.integration.list_pops
    ListProhibitedCircuitsHandlerArn  = local.integration.list_prohibited_circuits
    ListProhibitedWanPopsHandlerArn   = local.integration.list_prohibited_wan_pops
    ListRegionsHandlerArn             = local.integration.list_regions
    ListRiddenFiberHandlerArn         = local.integration.list_ridden_fiber
    ListRunRegionsHandlerArn          = local.integration.list_run_regions
    ListSitesHandlerArn               = local.integration.list_sites
    ListWanPopsHandlerArn             = local.integration.list_wan_pops
    ListWanSynthesesHandlerArn        = local.integration.list_wan_syntheses
    ...
    ReadRunRegionHandlerArn           = local.integration.read_run_region
    ReadSiteHandlerArn                = local.integration.read_site
    ReadWanPopHandlerArn              = local.integration.read_wan_pop
    ReadWanSynthesisHandlerArn        = local.integration.read_wan_synthesis
