...
def _delete_fiber_segment(carrier: str = "1", fiber_segment: str = "3") -> Dict[str, Any]:
    return {**_get_fiber_segment(carrier, fiber_segment), "httpMethod": "DELETE"}
...
def span_removed_fixture(
    ...
    store.items.extend(carriers)
    return answer(_delete_fiber_segment())
...
def test_a_removed_fiber_segment_is_no_longer_listed(served: Served) -> None:
    assert served(_get_fiber_segments()) == [DEN_ORD]


@pytest.mark.usefixtures("span_removed")
def test_a_removed_fiber_segment_is_no_longer_served(answer: Handler) -> None:
    assert answer(_get_fiber_segment())["statusCode"] == 404
...
def test_removing_a_fiber_segment_of_an_id_that_is_not_a_number_answers_404(
    ...
    assert answer(_delete_fiber_segment(carrier))["statusCode"] == 404
...
def test_removing_an_unknown_fiber_segment_answers_404(
    ...
    store.items.extend(carriers)
    assert answer(_delete_fiber_segment("1", "2"))["statusCode"] == 404
...
def test_removing_an_unknown_fiber_segment_names_the_fiber_segment(
    ...
    store.items.extend(carriers)
    assert served(_delete_fiber_segment("1", "2"))["error"] == "No such fiber segment"
...
def test_removing_an_unknown_fiber_segment_removes_nothing(
    ...
    store.items.extend(carriers)
    answer(_delete_fiber_segment("1", "2"))
    assert len(store.items) == len(carriers)
...
def test_removing_a_fiber_segment_id_that_is_not_a_number_answers_404(
    ...
    store.items.extend(carriers)
    assert answer(_delete_fiber_segment("1", fiber_segment))["statusCode"] == 404
...
def test_removing_a_fiber_segment_id_that_is_not_a_number_asks_the_store_nothing(
    ...
    answer(_delete_fiber_segment("1", fiber_segment))
...
def test_a_store_that_refuses_the_span_removal_answers_500(
    ...
    store.failing = True
    assert answer(_delete_fiber_segment())["statusCode"] == 500
...
def test_a_store_that_refuses_the_span_removal_names_the_error(
    ...
    store.failing = True
    assert served(_delete_fiber_segment())["error"] == "Failed to delete the fiber segment"
