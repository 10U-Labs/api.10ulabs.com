...
def test_an_empty_store_answers_200(answer: Answer) -> None:
...
def test_a_store_that_refuses_the_read_answers_500(answer: Answer, store: SimpleNamespace) -> None:
    store.failing = True
...
def test_another_resource_answers_404(answer: Answer) -> None:
...
def test_an_id_that_is_not_a_number_answers_404(answer: Answer, carrier: str) -> None:
...
def test_a_carrier_is_created_with_201(answer: Answer) -> None:
...
def test_the_created_carrier_is_located_under_the_collection(answer: Answer) -> None:
...
def test_the_created_carrier_is_then_listed(answer: Answer, served: Served) -> None:
    answer(_post({"name": "lumen"}))
...
def test_a_body_that_is_not_exactly_a_name_answers_400(answer: Answer, body: Any) -> None:
    assert answer(_post(body))["statusCode"] == 400


def test_a_body_that_is_not_json_answers_400(answer: Answer) -> None:
...
def test_a_refused_body_writes_nothing(answer: Answer, store: SimpleNamespace) -> None:
    answer(_post({}))
    assert store.items == []


def test_a_store_that_refuses_the_write_answers_500(answer: Answer, store: SimpleNamespace) -> None:
    store.failing = True
...
def test_renaming_an_unknown_carrier_answers_404(answer: Answer) -> None:
...
@pytest.mark.parametrize("carrier", ["#", "", "lumen", "-1"])
def test_renaming_an_id_that_is_not_a_number_answers_404(answer: Answer, carrier: str) -> None:
...
def test_a_rename_that_is_not_json_answers_400(answer: Answer) -> None:
    event = {**_put({}), "body": "{"}
    assert answer(event)["statusCode"] == 400
...
def test_a_refused_rename_changes_nothing(
    ...
    store.items.extend(carriers)
    answer(_put({}))
    assert (store.updates, stored("2")["name"]) == ([], {"S": "zayo"})
...
@pytest.mark.usefixtures("deleted")
def test_a_deleted_carrier_is_no_longer_served(answer: Answer) -> None:
...
def test_deleting_an_unknown_carrier_answers_404(answer: Answer) -> None:
...
@pytest.mark.parametrize("carrier", ["#", "", "lumen", "-1"])
def test_deleting_an_id_that_is_not_a_number_answers_404(answer: Answer, carrier: str) -> None:
